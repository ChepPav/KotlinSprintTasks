...
fun main() {
    ...
    val actualShipCondition = readln().toBoolean()
    ...
    val actualWether = readln().toBoolean()
    println(
        """
        Может ли корабль выйти в плавание? 
        ${
            actualShipCondition == SHIP_CONDITION &&
                    actualCrewNumber in MIN_CREW_NUMBERS..MAX_CREW_NUMBERS &&
                    actualProvisions > PROVISIONS &&
                    (actualWether == GOOD_WEATHER || actualWether != GOOD_WEATHER) ||
                    ((actualShipCondition == SHIP_CONDITION || actualShipCondition != SHIP_CONDITION) &&
